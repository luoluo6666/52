        ...
        return {
            ...
            getDeviceTabel:function (data) {
                ...
                return pageInit = {
                    ...
                    isAdd:'添加',
                    isIndex:'',
                    add:'field.deviceAdd({id:item.id})',
                    table:{
                        tableHead:['ID','设备名称','生产厂家','当前状态','操作'],
                        tableData: data,
                        tableName:[
                            {type:'text',value: '{{item.id}}'},
                            {type:'text',value: '{{item.name}}'},
                            {type:'text',value: '{{item.factory}}'},
                            {type:'text',value: '{{item.status|deviceFilter:"status"}}'}
                        ],
                        tableButton:[
                            {btnStyle:'btn btn-default btn-xs',btnUi_sref:'field.deviceAdd({edit:item.id})',btnName :'编辑' },
                            {btnStyle:'btn btn-default btn-xs',btnUi_sref:'field.deviceAdd({id:item.id})',btnName :'查看' },
                            {ngClass:'{\'orange\':item.status==1 }',btnStyle:'btn btn-primary btn-xs',btnClick:'vm.changeStatus(item.id,(item.status==0?1:0))',btnNameBind:"item.status==0?1:0|deviceFilter:'status'" },
                            {btnStyle:'btn btn-danger btn-xs',btnClick:'vm.delMsg(item.id)',btnName :'删除' }
                        ]
                    }
                }
            },
            //信息页面
            getMsgTable:function (data) {
                var pageInit = null;
                return pageInit = {
                    tableTitle:'消息管理',
                    isSort:'',
                    ...
                    add:'field.MsgAdd',
                    table:{
                        tableHead:['ID','标题','发送人群','创建者','发送时间','发送状态','操作'],
                        tableData: data,
                        tableName:[
                            {type:'text',value: '{{item.id}}'},
                            {type:'text',value: '{{item.title}}'},
                            {type:'text',value: '{{item.personType|personTypeFilter:"personType"}}'},
                            {type:'text',value: '{{item.createByName}}'},
                            {type:'text',value: '{{item.pushAt|date:"yyyy-MM-dd HH:mm:ss"}}'},
                            {type:'text',value: '{{item.status|customerFilter:"sendStatus"}}',tdNgClass:'{\'text-red\':item.status==0,\'text-green\':item.status!=0}'}
                        ],
                        tableButton:[
                            {ngClass:'{\'unvisible\':item.status==1 }',btnStyle:'btn btn-default btn-xs text-green ',btnClick:'vm.send($index)',btnName :'立即发送' },
                            {ngClass:'{\'unvisible\':item.status==1}',btnStyle:'btn btn-default btn-xs ',btnUi_sref:'field.MsgAdd({edit:item.id})',btnName :'编辑' },
                            {btnStyle:'btn btn-default btn-xs',btnUi_sref:'field.MsgAdd({id:item.id})',btnName :'查看' },
                            {btnStyle:'btn btn-danger btn-xs',btnClick:'vm.delete($index)',btnName :'删除' }
                        ]
                    }
                }
            },
            //常量页面
            getConstantTable:function (data) {
                var pageInit = null;
                return pageInit = {
                    tableTitle:'常量表',
                    isSort:'',
                    isAdd:'添加',
                    isIndex:'',
                    add:'field.parameterSettingEdit',
                    table:{
                        tableHead:['序号','类别','名称','值','创建时间','更新时间','备注','操作'],
                        tableData: data,
                        tableName:[
                            {type:'text',value: '{{item.id}}'},
                            ...
                            {type:'text',value: '{{item.createAt|date:"yyyy-MM-dd HH:MM"}}'},
                            {type:'text',value: '{{item.updateAt|date:"yyyy-MM-dd HH:MM"}}'},
                            {type:'text',value: '{{item.comments}}'}
                        ],
                        tableButton:[
                            {btnStyle:'btn btn-default btn-xs',btnUi_sref:'field.parameterSettingEdit({id:item.id})',btnName :'编辑' }
                        ]
                    }
                }
            },
            //用户管理
            getUserTable:function (data) {
                var pageInit = null;
                return pageInit = {
                    tableTitle:'用户列表',
                    isSort:'',
                    isAdd:'',
                    isIndex:'',
                    add:'field.userManagementDetails({id:item.id})',
                    table:{
                        tableHead:['ID','手机号码','注册时间','付费状态','冻结状态','操作'],
                        tableData: data,
                        tableName:[
                            {type:'text',value: '{{item.id}}'},
                            {type:'text',value: '{{item.mobile}}'},
                            {type:'text',value: '{{item.createAt|date:"yyyy-MM-dd HH:MM"}}'},
                            {type:'text',value: '{{item.buyStatus|customerFilter:"buyStatus"}}'},
                            {type:'text',value: '{{item.status|customerFilter:"lockStatus"}}'}
                        ],
                        tableButton:[
                            {btnStyle:'btn btn-default btn-xs',btnUi_sref:'field.userManagementDetails({id:item.id})',btnName :'查看' },
                            {btnStyle:'btn btn-danger btn-xs',btnClick:'vm.changeStatus(item.id,item.status==0?item.status=1:0)',btnNameBind:"item.status==0?1:0|customerFilter:'status'" }
                        ]
                    }
                }
            },
